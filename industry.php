...
<table id="example" class="hover section-subtitle wow fadeIn" style="width:auto">
  ...
  <tbody>
    ...
    <tr class="section-subtitle wow fadeIn">
      ...
      <td> <?php echo $tableData['industry']; ?> </td>
      <td> <?php echo $tableData['no. of businesses']; ?> </td>
      <td> <?php echo $tableData['year']; ?> </td>
      </tr>
    <?php endforeach; ?>
  </tbody>
</table> 
...
{
  global $conn;
  if($stmt = $conn->prepare("SELECT name, industry, `no. of businesses`, `year` FROM industry_df"))
  {
    $stmt->execute();
    $stmt->bind_result($name, $industry, $numBus, $year);
    while($stmt->fetch()){
      $rows [] = array('name' => $name, 'industry' => $industry, 'no. of businesses' => $numBus, 'year' => $year);
    }
    $stmt->close();
   // print_r($rows);
    return $rows;
